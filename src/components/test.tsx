"use client";

import React, { useState, useEffect } from "react";
import Link from "next/link";
import Image from "next/image";
import menuIcon from "../../public/iconamoon_menu-burger-horizontal.svg";
import UIButton from "@/ui/UIButtonWithModal";

export default function Header() {
  const [menuOpen, setMenuOpen] = useState(false);
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") setMenuOpen(false);
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, []);
  const [langOpen, setLangOpen] = useState(false);
  const [lang, setLang] = useState("ru");
  const LANGS = [
    { code: "ru", label: "RU" },
    { code: "en", label: "EN" },
    { code: "ua", label: "UA" },
  ];
  return (
    <header className="w-full container-mobile container-desktop pt-[51px] flex items-center justify-between bg-transparent relative z-20 md:w-max-[1280px]">
      {/* Desktop flex row: logo | nav | lang | button */}
      <div className="flex items-center w-full justify-between">
        <div className="flex items-center gap-2">
          <Link href="/" className="third-family text-lg uppercase font-bold">
            Aleko <span className="gradient-text">Sokurashvili</span>
          </Link>
        </div>
        <nav className="hidden md:flex items-center gap-4">
          <Link href="/structure" className="text-sm font-medium">
            Структура
          </Link>
          <Link href="/about" className="text-sm font-medium">
            Обо мне
          </Link>
          <Link href="/Benefits" className="text-sm font-medium">
            Плюсы
          </Link>
          <Link href="/Reviews" className="text-sm font-medium">
            Отзывы
          </Link>
          <Link href="/FAQ" className="text-sm font-medium">
            FAQ
          </Link>
        </nav>
        <div className="hidden md:flex items-center ml-4 relative">
          <button
            className="flex items-center gap-1 px-2 py-1  text-sm font-family font-semibold text-white hover:bg-white/20 transition-colors select-none"
            onClick={() => setLangOpen((v) => !v)}
            aria-haspopup="listbox"
            aria-expanded={langOpen}
          >
            {LANGS.find((l) => l.code === lang)?.label}
            <svg
              className={`w-4 h-4 ml-1 transition-transform ${
                langOpen ? "rotate-180" : ""
              }`}
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
            >
              <path d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <ul
            className={`absolute right-0 mt-1 w-16 rounded bg-[#22223b] shadow-lg overflow-hidden transition-all duration-200 z-30
              ${
                langOpen
                  ? "opacity-100 scale-100 pointer-events-auto"
                  : "opacity-0 scale-95 pointer-events-none"
              }`}
            style={{ boxShadow: "0 4px 16px 0 rgba(0,0,0,0.12)" }}
            role="listbox"
          >
            {LANGS.map((l) => (
              <li
                key={l.code}
                className={`px-3 py-2 text-sm cursor-pointer hover:bg-[#393a5a] text-white transition-colors ${
                  lang === l.code ? "font-bold" : ""
                }`}
                onClick={() => {
                  setLang(l.code);
                  setLangOpen(false);
                }}
                role="option"
                aria-selected={lang === l.code}
              >
                {l.label}
              </li>
            ))}
          </ul>
        </div>
        <UIButton variant="primary" className="hidden md:inline-block ml-4">
          Купить со скидкой
        </UIButton>
      </div>
      <div className="relative md:hidden">
        <button
          className="flex items-center gap-1 px-2 py-1  text-sm font-family font-semibold text-white hover:bg-white/20 transition-colors select-none"
          onClick={() => setLangOpen((v) => !v)}
          aria-haspopup="listbox"
          aria-expanded={langOpen}
        >
          {LANGS.find((l) => l.code === lang)?.label}
          <svg
            className={`w-4 h-4 ml-1 transition-transform ${
              langOpen ? "rotate-180" : ""
            }`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
          >
            <path d="M19 9l-7 7-7-7" />
          </svg>
        </button>

        <ul
          className={`absolute right-0 mt-1 w-16 rounded bg-[#22223b] shadow-lg overflow-hidden transition-all duration-200 z-30
            ${
              langOpen
                ? "opacity-100 scale-100 pointer-events-auto"
                : "opacity-0 scale-95 pointer-events-none"
            }`}
          style={{ boxShadow: "0 4px 16px 0 rgba(0,0,0,0.12)" }}
          role="listbox"
        >
          {LANGS.map((l) => (
            <li
              key={l.code}
              className={`px-3 py-2 text-sm cursor-pointer hover:bg-[#393a5a] text-white transition-colors ${
                lang === l.code ? "font-bold" : ""
              }`}
              onClick={() => {
                setLang(l.code);
                setLangOpen(false);
              }}
              role="option"
              aria-selected={lang === l.code}
            >
              {l.label}
            </li>
          ))}
        </ul>
      </div>
      <button
        className="flex justify-center items-center w-8 h-8 md:hidden z-20"
        onClick={() => setMenuOpen(!menuOpen)}
        aria-label="Toggle Menu"
      >
        <Image src={menuIcon} alt="Menu" width={35} height={35} />
      </button>
      <nav
        className={`fixed top-0 right-0 h-[50%] w-full max-w-lg bg-black/10 backdrop-blur-lg shadow-lg z-10 flex flex-col items-start pt-20 px-6 gap-6 transition-transform duration-300 md:hidden ${
          menuOpen ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        <Link
          href="/structure"
          className="text-lg font-medium text-white"
          onClick={() => setMenuOpen(false)}
        >
          Структура
        </Link>
        <Link
          href="/about"
          className="text-lg font-medium text-white"
          onClick={() => setMenuOpen(false)}
        >
          Обо мне
        </Link>
        <Link
          href="/Benefits"
          className="text-lg font-medium text-white"
          onClick={() => setMenuOpen(false)}
        >
          Плюсы
        </Link>
        <Link
          href="/Reviews"
          className="text-lg font-medium text-white"
          onClick={() => setMenuOpen(false)}
        >
          Отзывы
        </Link>
        <Link
          href="/FAQ"
          className="text-lg font-medium text-white"
          onClick={() => setMenuOpen(false)}
        >
          FAQ
        </Link>
      </nav>
      {menuOpen && (
        <div
          className="fixed inset-0 bg-black/30 z-0 md:hidden"
          onClick={() => setMenuOpen(false)}
        ></div>
      )}
    </header>
  );
}
